...
import model.beit.utils as utils
from model.beit.modeling_utils import BEiT3Wrapper, _get_base_config, _get_large_config
...
class BEiT3ForRetrievalOri(BEiT3Wrapper):
    def __init__(
            self, 
            args,
            **kwargs
    ):
        super(BEiT3ForRetrievalOri, self).__init__(args=args)
        embed_dim = args.encoder_embed_dim
        self.language_head = nn.Linear(embed_dim, embed_dim, bias=False)
        self.vision_head = nn.Linear(embed_dim, embed_dim, bias=False)
        self.language_head.apply(self._init_weights)
...
@register_model
def beit3_base_patch16_224_retrieval_ori(pretrained=False, **kwargs):
    args = _get_base_config(**kwargs)
    model = BEiT3ForRetrievalOri(args, **kwargs)
    return model


@register_model
def beit3_base_patch16_384_retrieval_ori(pretrained=False, **kwargs):
    args = _get_base_config(img_size=384, **kwargs)
    model = BEiT3ForRetrievalOri(args, **kwargs)
    return model
...
@register_model
def beit3_large_patch16_224_retrieval_ori(pretrained=False, **kwargs):
